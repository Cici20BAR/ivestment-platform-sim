...
export function simulateInvestment(p) {
    ...
    for (let m = 1; m <= months; m++) {
        ...
        if (p.annualContribution && m % 12 === 0) {
...
export function monteCarlo(runs, rateMin, rateMax, base) {
    const results = [];
...
export function exportCsv(file, sim) {
    ...
    sim.yearlySnapshots?.forEach(snapshot => {
        rows.push(`${snapshot.year},${snapshot.value.toFixed(2)},${snapshot.invested.toFixed(2)},${snapshot.grossProfit.toFixed(2)},${snapshot.tax.toFixed(2)},${snapshot.netProfit.toFixed(2)}`);
    });
