...
if (isDocker) {
    console.log("🚀 Rulez în interiorul unui container Docker!");
}
else {
...
function assertNumber(name, value, allowZero = true) {
...
if (getArgs("--annual")) {
    ...
    params.annualContribution = annualContrib;
}
...
if (!args.includes("--compare") && !args.includes("--scenarios") && !args.includes("--montecarlo")) {
    ...
    if (params.annualContribution)
        console.log(` Contributie anuala: ${params.annualContribution} RON`);
